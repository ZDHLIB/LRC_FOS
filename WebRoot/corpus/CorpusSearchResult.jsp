		...
		<form action="<%=request.getContextPath()%>/corpus/corpusServlet?method=deleteTextdetail" method="post" name="corpusUploadForm" id="corpusUploadForm" onSubmit="return(FormCheck())">
			<table width="100%" border="0" cellspacing="1" cellpadding="3" align="center" class="tableBorder" onclick="javascript:checkT_F()">
				...
				<tr>
					...
					<td width="100" class="forumRow">
						<center>
							<a href="Besure.jsp?path=<%=new String(alltextdetail.get(i).getContent().getBytes(),"UTF-8")%>&view=<%=u.getView()%>&viewed=<%=u.getViewed()%>&userID=<%=u.getId()%>&username=<%=u.getLoginName()%>&fileID=<%=alltextdetail.get(i).getId()%>">查看</a> |
							<a href="Besuredown.jsp?&path=<%=new String(alltextdetail.get(i).getContent().getBytes(),"UTF-8")%>&down=<%=u.getDown()%>&downed=<%=u.getDowned()%>&userID=<%=u.getId()%>&username=<%=u.getLoginName()%>&fileID=<%=alltextdetail.get(i).getId()%>">下载</a>
						</center>
					</td>
				</tr>
				<%
			...
			<center>
		  		...
		  		<input type="reset" name="button" id="button" value="检索课文" onclick="location.replace('CorpusSearch.jsp')"></center>
			<br>
			<center>
				<%
