...
<html>
	...
	<body>
		<div align="right">
			<a href="<%=request.getContextPath()%>/admin/corpus/AdminCorpusSearch3.jsp">返回</a>
		</div>
		<br>
		<table width="95%" border="0" cellspacing="1" cellpadding="3" align="center" class="tableBorder">
			...
			<%
				...
				String idAx[] =  new String[200000];
				for(int i=0; i<idAx.length; i++) {
					idAx[i] = new String();
				}
				String idAy[] =  new String[200000];
				...
				if(corpusList!=null){
					...
					while(xc.hasNext())
					{
						...
						if(temp.indexOf("-") == -1){
						idAx[x] = String.valueOf(tempCorpusBean.getId());					
							x++;
						}else{
						idAy[y] = String.valueOf(tempCorpusBean.getId());
							y++;
			...
			<%
				...
				String idAllx = new String();
				idAllx = idAx[0];
				for(int i=1;i<x;i++){
					idAllx += "-"+idAx[i];
				} 
				String nxx = String.valueOf(x);
				String idAlly = new String();
				idAlly = idAy[0];
				for(int i=1;i<y;i++){
					idAlly += "-"+idAy[i];
				} 
				String nxy = String.valueOf(y);
		...
		<br><center>
			<form id="corpusSearchForm" name="corpusSearchForm" method="post"
				action="<%=request.getContextPath()%>/corpus/corpusServlet?method=searchCorpus8&idAll=<%=idAll%>&number=<%=nx%>&idAllx=<%=idAllx%>&numberx=<%=nxx%>&idAlly=<%=idAlly%>&numbery=<%=nxy%>">
				<table>
					<tr>
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<td>
		...
		<center>
			<%
				out.print(corpusBean.PageLegend2()+",其中人工语料"+x+"条，网络语料"+y+"条");
			%>
		</center>
	</body>
</html>
